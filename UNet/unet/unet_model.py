...
class UNet(nn.Module):
    def __init__(self):
        ...
        # Expansive path
        self.dec5_1 = CBR(in_channels=1024, out_channels=512)

        self.unpool4 = nn.ConvTranspose2d(in_channels=512, out_channels=512,
                                          kernel_size=2, stride=2, padding=0, bias=True)

        self.dec4_2 = CBR(in_channels=2 * 512, out_channels=512)
        self.dec4_1 = CBR(in_channels=512, out_channels=256)

        self.unpool3 = nn.ConvTranspose2d(in_channels=256, out_channels=256,
                                          kernel_size=2, stride=2, padding=0, bias=True)

        self.dec3_2 = CBR(in_channels=2 * 256, out_channels=256)
        self.dec3_1 = CBR(in_channels=256, out_channels=128)

        self.unpool2 = nn.ConvTranspose2d(in_channels=128, out_channels=128,
                                          kernel_size=2, stride=2, padding=0, bias=True)

        self.dec2_2 = CBR(in_channels=2 * 128, out_channels=128)
        self.dec2_1 = CBR(in_channels=128, out_channels=64)

        self.unpool1 = nn.ConvTranspose2d(in_channels=64, out_channels=64,
                                          kernel_size=2, stride=2, padding=0, bias=True)

        self.dec1_2 = CBR(in_channels=2 * 64, out_channels=64)
